...
GLT_func_prefix void glt_ult_creation_syncvar_to(void(*thread_func)(void *), void * arg,
...
GLT_func_prefix void glt_syscall_end_blocking() {
    GLT_ERROR_QTH;
}

GLT_func_prefix int glt_syscall_accept(int socket,
        struct sockaddr * restrict address, socklen_t * restrict address_len) {
    GLT_ERROR_QTH;
    return -1;
}

GLT_func_prefix int glt_syscall_connect(int socket,
        struct sockaddr * restrict address, socklen_t address_len) {
    GLT_ERROR_QTH;
    return -1;
}

GLT_func_prefix int glt_syscall_poll(struct pollfd fds[], nfds_t nfds, int timeout) {
    GLT_ERROR_QTH;
    return -1;
}

GLT_func_prefix size_t glt_syscall_pread(int filedes, void *buf, size_t nbyte,
        off_t offset) {
    GLT_ERROR_QTH;
    return -1;
}

GLT_func_prefix size_t glt_syscall_read(int filedes, void *buf, size_t nbyte) {
    GLT_ERROR_QTH;
    return -1;
}

GLT_func_prefix size_t glt_syscall_pwrite(int filedes, void *buf, size_t nbyte,
        off_t offset) {
    GLT_ERROR_QTH;
    return -1;
}

GLT_func_prefix size_t glt_syscall_write(int filedes, void *buf, size_t nbyte) {
    GLT_ERROR_QTH;
    return -1;
}

GLT_func_prefix int glt_syscall_select(int nfds, fd_set * restrict readfds,
...
GLT_func_prefix int glt_syscall_system(const char * command) {
    GLT_ERROR_QTH;
    return -1;
}

GLT_func_prefix int glt_syscall_wait4(pid_t pid, int *stat_loc, int options,
